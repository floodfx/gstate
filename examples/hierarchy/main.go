...
func main() {
	...
	machine := gstate.New[S, E, any]("hierarchy").
		Initial("parent").
		State("parent", func(s *gstate.StateBuilder[S, E, any]) {
			// Set the default child to enter when 'parent' is entered.
			s.Initial("child1")
			
			// Actions defined on a parent run for ALL entries into children.
			s.Entry(func(c any) any { fmt.Println("[parent] Entering..."); return c })
			s.Exit(func(c any) any { fmt.Println("[parent] Exiting..."); return c })
			...
			s.State("child1", func(s *gstate.StateBuilder[S, E, any]) {
				s.Entry(func(c any) any { fmt.Println("  [child1] Entering..."); return c })
				s.Exit(func(c any) any { fmt.Println("  [child1] Exiting..."); return c })
				s.On("TO_CHILD2").GoTo("child2")
			})

			s.State("child2", func(s *gstate.StateBuilder[S, E, any]) {
				s.Entry(func(c any) any { fmt.Println("  [child2] Entering..."); return c })
				s.Exit(func(c any) any { fmt.Println("  [child2] Exiting..."); return c })
				s.On("TO_CHILD1").GoTo("child1")
			})
		}).
		State("done", func(s *gstate.StateBuilder[S, E, any]) {
			s.Type(gstate.Final)
		}).
		Build()
